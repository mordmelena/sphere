...
define(["exports", "data", "glMatrix"], function(data, exports) {
	...
	function calcuateVertexNormalsFromPolygonNormals(){
		...
		for(var v = 0; v < this.vertices.length; v++) {
			...
			var vn = this.vertexNormals[v];
			for(var p = 0; p < this.polygonVertices.length; p++) {
				var polygon = this.polygonVertices[p];
				for(var vp = 0; vp < polygon.length; vp++){
					// console.log(polygon.length);
					if (polygon[vp] == v){
						vn[0] += this.polygonNormals[p][0];
						vn[1] += this.polygonNormals[p][1];
						vn[2] += this.polygonNormals[p][2];
					}

				}
			}
			// console.log(vn[0]);
			this.vertexNormals[v][0] = vn[0]/Math.sqrt(vn[0]**2 + vn[1]**2 + vn[2]**2);
			this.vertexNormals[v][1] = vn[1]/Math.sqrt(vn[0]**2 + vn[1]**2 + vn[2]**2);
			this.vertexNormals[v][2] = vn[2]/Math.sqrt(vn[0]**2 + vn[1]**2 + vn[2]**2);
		}
	...
	function calculateNormalForPolygon(vertices, polygon, n){
		...
		var zero = vertices[polygon[0]];
		var e1 =  vertices[polygon[1]];

		var e1x = e1[0] - zero[0];
		var e1y = e1[1] - zero[1];
		var e1z = e1[2] - zero[2];

		var e2 = vertices[polygon[2]];
		...
		// We do not use the matrix lib here.
			...
			var invLength = 1/Math.sqrt(n[0]**2 + n[1]**2 + n[2]**2);
			n[0]  *= invLength;
			n[1]  *= invLength;
			n[2]  *= invLength;
