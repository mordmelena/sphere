...
// Decision variable.
    ...
    function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
        ...
        // lastDerivative = calcDerivative(polygon[lastIndex-1], polygon[lastIndex]);
// check this func

        var prevVertexIndex = 0;
        for (var a = lastIndex; a > 0; a--) {
            var st = vertices[polygon[a]];
...
// Also after the rasterization with floor we need a valid triangle.
        ...
        for(var v = 0; v < polygon.length; v++) {
            ...
            var nextVertexIndex = (v < lastIndex) ? v + 1 : 0;
    ...
    function scanlineFillPolygon(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
        ...
        // BEGIN exercise Scanline
            for (let y = 0; y < scanlineIntersection.length; y++) {
                let line = scanlineIntersection[y];
                if (!line) {
                    continue;
                }
                ...
                for (let j = 0; j < line.length - 1; j+=2) {
                    ...
                    for (let x = line[j].x ; x <= line[j + 1].x ; x++) {
                        // 			 // z += AdivC;
                        framebuffer.set(x, y, getZ(x, y) , color);
                ...
                // for (let j = 0; j < line.length - 1; j+=2) {
                //     var currentInterpolationA = scanlineIntersection[y][j].x;
                //     var currentInterpolationB = scanlineIntersection[y][j+1].x;
                //
                //             // Fill line section inside polygon, loop x.
                //
                //             for (var x = currentInterpolationA; x <= currentInterpolationB; x++) {
                //                 framebuffer.set(x, y, getZ(x, y), color);
        ...
        //
        //         var currentInterpolationA = scanlineIntersection[y][i].x;
        //         var currentInterpolationB = scanlineIntersection[y][i+1].x;
        //
        //         // Fill line section inside polygon, loop x.
        //
        //         for (var x = currentInterpolationA; x <= currentInterpolationB; x++) {
