...
define(["exports", "scene"], function(exports, scene) {
	...
	function zBufferTest(x, y, z, color) {
		...
		if(current == maxDistance) {
			zBuf[indexZBuf] = z;
			// console.log("zbuf" + zBuf[indexZBuf] +" , z" + z);
		return true;
		}
		...
		if ((z-zBuf[indexZBuf] <= -epsilon) || (z-zBuf[indexZBuf] >= epsilon)) {
		...
		return false;
		//
		// if (z - current >= 0.1 || z - current <= -0.1) {
		// 	if (z - current > 0) {
		// 	// console.log("zbuf   " + zBuf[indexZBuf] +" , z   " + z + " current  "+ current);
		// 	zBuf[indexZBuf] = z;
		// 	return true;
		// }
		// 	}
