...
define(["exports", "data", "glMatrix"], function(exports, data) {
	...
	function devide_all(recursionDepth, nbRecusions) {
		...
		for (var p = 0; p < this.polygonVertices.length; p++) {
			...
			for (var v = 0; v < 3; v++ ){
				var v1 = this.vertices[this.polygonVertices[p][v]];
				var v2 = this.vertices[this.polygonVertices[p][v+1]];
				if (v==2){
					v2 = this.vertices[this.polygonVertices[p][0]];
				...
				var newVertex = [
					x * (1 / Math.sqrt(x**2 + y**2 + z**2)),
					y * (1 / Math.sqrt(x**2 + y**2 + z**2)),
					z * (1 / Math.sqrt(x**2 + y**2 + z**2))
					];
				this.vertices.push(newVertex);

				var newIndex = this.vertices.length - 1;
				newIndices.push(newIndex);
			}
			//Reihenfolge!
			sphereVertices.push([newIndices[0], newIndices[1], newIndices[2]],
				[this.polygonVertices[p][0], newIndices[0], newIndices[2]],
				[this.polygonVertices[p][1], newIndices[1], newIndices[0]],
				[this.polygonVertices[p][2], newIndices[2], newIndices[1]]
			);
		}
